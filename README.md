...
- AI 总结：连接用户自己的 OpenAI 兼容接口；通用摘要会自动识别会议、访谈、讲座、通话、备忘或其他内容并调整分析重点，同时保留“会议纪要”模式；分析结果可导出 Word、PDF、TXT。API Key 仅保存在本机受限文件中，原始音频不会发送给 AI 服务。
...
在「设置 → AI 服务」中填写 OpenAI 兼容接口地址、API Key 和模型名称。生成总结时，WordGrab 只发送当前文稿文字和时间点，不发送原始音频或其他历史文稿。API Key 保存在仅当前 macOS 用户可读的本机文件中，不写入项目配置或日志。
